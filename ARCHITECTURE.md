...
| R29 | **Revocation is not yet effective until the daemon restarts** | Certain | High | §3.6 promises that removing a device's key takes effect on the next packet. `serve_iroh` does not deliver that: it reads `list_active()` once at startup and builds a fixed `Allowlist` from that snapshot, so a device revoked while the daemon is running keeps connecting until `gonomad serve` is restarted. Newly paired devices are equally invisible until restart. The gap is in the daemon's wiring, not the policy layer — `Allowlist` is consulted per handshake, it is simply consulted against stale data. Fix is to have the transport policy read through to the store on each handshake, or to hold the allowlist behind a watch channel the device commands update. **Until then, treat `gonomad devices revoke` as requiring a restart to take effect.** Must close before any release that claims remote revocation. |
