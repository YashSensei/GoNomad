...
| R29 | Revocation is effective on the next **connection**, not the next packet | Certain | Medium | §3.6 promises the next *packet*; the daemon delivers the next *connection*. `LiveAllowlist` re-reads `list_active()` from SQLite on every handshake and fails closed if the database is unreadable, so a revoked device cannot reconnect and a newly paired one needs no restart — the original bug, where a startup snapshot made `gonomad devices revoke` a no-op until restart, is closed and asserted by `revoking_a_device_takes_effect_without_restarting_the_daemon`. The read is uncached deliberately: any TTL is a window in which a revoked key still works. **What remains:** a device already holding an open connection keeps it, because authorisation is checked at handshake time and its `DeviceGrant` is a per-connection snapshot. Closing that needs either a periodic sweep that drops connections whose device is no longer active, or a re-check on each request — and the latter cannot be a database read per request, because `pty.screen` polling would turn it into a hot path. A sweep is the intended fix. Note revocation must be performed by a separate process (`gonomad devices revoke`) editing the same SQLite file, which is why a fresh read is required rather than an in-process channel. |
