...
| R24 | **A wrong pairing PSK is detected by the phone, not the daemon** | Certain | High | In `IKpsk2` the pre-shared key is mixed into the *responder's* message, so the daemon completes its side of the handshake and briefly holds a session whose keys do not match the peer's. The session is unusable — the first record fails to authenticate — but it exists. **Consequence: device registration must never be triggered by handshake completion.** It must wait for an authenticated application-level exchange over the control stream. Getting this wrong would register an attacker who guessed nothing. Documented in `gonomad-transport/src/tcp.rs` and asserted by `pairing_with_the_wrong_secret_is_refused`. |
| R25 | The TCP binding caps a single frame at 256 KiB, against the protocol's 32 MiB | Certain | Medium | Structural, not tunable: mux credit is returned on whole-frame consumption, so a frame larger than the receive window can never be reassembled — the sender waits for credit the receiver cannot return. `MuxConfig::validate()` enforces `initial_window ≥ max_frame_len + HEADER_LEN`. Larger responses must chunk across frames with `FrameFlags::LAST`. **The cap disappears when iroh lands**, because QUIC flow-controls a byte stream rather than whole messages. |
| R26 | Mux window configuration must match on both ends | Medium | Low | The receiver *enforces* the window rather than trusting the sender, so an `initial_window` disagreement tears the connection down. QUIC negotiates this in its transport parameters; here it is a shared constant, so a version-skewed pair can fail to connect with no obvious cause. Fold into the hello negotiation (§10.4) before v0.1.0. |
...
| R27 | One Noise session per QUIC stream costs a round trip per stream | Certain | Low | `snow` advances an implicit nonce per direction, so a session shared across independent streams serialises them and rebuilds the head-of-line blocking QUIC was chosen to avoid (§4.4). The chosen alternative — a session per stream — pays one extra handshake round trip whenever a stream opens. Acceptable because streams are long-lived: one per terminal, one per transfer, so the cost lands on opening a terminal tab and never on a keystroke. Removing it needs explicit per-record nonces instead of `snow`'s implicit counter, which is a change to the security core and wants review, not a refactor. |
| R29 | Revocation takes effect within ~5 s, not literally on the next packet | Certain | Low | §3.6 says "next packet"; the implementation delivers **immediately for new connections and within `REVOCATION_SWEEP` (5 s) for one already open**, which is what that promise means in practice. Two mechanisms. `LiveAllowlist` re-reads `list_active()` from SQLite on every handshake, so a revoked device cannot reconnect and a newly paired one needs no restart; it fails closed on an unreadable database or a poisoned lock, and is uncached on purpose because any TTL is a window in which a revoked key still works. `sweep_once` then re-checks live connections every 5 s and closes any whose device is no longer active, which is what reaches a session that was already established — without it, revoking a device mid-terminal-session left that session working for as long as the user kept it open. A literal per-packet check was rejected rather than deferred: `pty.screen` polling would put an indexed SQLite read in the terminal hot path, so its cost would scale with typing speed, whereas a sweep costs one read per interval regardless of load. Asserted end to end by `revoking_a_device_closes_the_connection_it_already_holds`, over a real iroh connection. Note revocation is performed by a *separate process* (`gonomad devices revoke`) editing the same SQLite file, which is why a fresh read is required and no in-process channel would do. Residual risk is the ≤5 s window and is accepted. |
| R28 | Dropping an iroh `Endpoint` silently kills every connection on it | Certain | Medium | iroh's `Endpoint::drop` aborts all connections and reports it with a `tracing::error!` only — the connection handles keep looking valid and then fail on next use. A connection is therefore only as alive as the endpoint it was taken from, and that ownership is invisible in the type system. This already caused one bug: `establish()` returned a connection while its endpoint fell out of scope, and the session died milliseconds later as a nondeterministic `ConnectionLost` on the first request. Fixed by having the session own the endpoint and close it deliberately. **Any future code that binds an endpoint must keep it alive for the whole life of anything derived from it**; there is no compiler help here. |
